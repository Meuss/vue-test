...
const app = new Vue({
  ...
  computed: {
    filteredList() {
      return this.champList.filter(post => post.title.toLowerCase().includes(this.keyword.toLowerCase()));
    },
    filteredListAPI() {
      return this.champListAPI.filter(post => post.title.toLowerCase().includes(this.keyword.toLowerCase()));
    },
  },
});
